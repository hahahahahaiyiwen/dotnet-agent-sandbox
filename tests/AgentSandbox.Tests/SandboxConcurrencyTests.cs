...
public class SandboxConcurrencyTests
{
    ...
    [Fact]
    public async Task IsolatedTimeout_OverlappingBackgroundCommands_DoNotClearNewerTimeoutTracking()
    {
        using var sandbox = new Sandbox(options: new SandboxOptions
        {
            EnableIsolatedParallelCommandExecution = true,
        ...
        var first = Task.Run(() => sandbox.Execute("pslow 200"));
        await Task.Delay(10);
        var second = Task.Run(() => sandbox.Execute("pslow 500"));

        var firstTimeout = await first;
        var secondTimeout = await second;
        Assert.False(firstTimeout.Success);
        Assert.False(secondTimeout.Success);
        Assert.Contains("timed out", firstTimeout.Stderr, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("timed out", secondTimeout.Stderr, StringComparison.OrdinalIgnoreCase);
